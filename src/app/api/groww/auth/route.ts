...
import { growwProfile } from "@/lib/groww";
...
export async function POST(req: NextRequest) {
  ...
  const { apiKey, accessToken } = body as Record<string, string>;
  if (!accessToken) {
    return NextResponse.json({ status: "invalid", error: "Access token is required" }, { status: 400, headers: HEADERS });
  }

  const result = await growwProfile({ broker: "groww", groww: { apiKey: apiKey ?? "", accessToken } });
