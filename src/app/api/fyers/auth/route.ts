...
import { fyersProfile } from "@/lib/fyers";
...
export async function POST(req: NextRequest) {
  ...
  const { appId, secretKey, accessToken } = body as Record<string, string>;
  if (!accessToken) {
    return NextResponse.json({ status: "invalid", error: "Access token is required" }, { status: 400, headers: HEADERS });
  }

  const result = await fyersProfile({ broker: "fyers", fyers: { appId: appId ?? "", secretKey: secretKey ?? "", accessToken } });
